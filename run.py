...
def main():
    ...
    p.add_argument("--input", "-i", type=Path, help="input CSV file (optional). If omitted, looks in data/ or ~/Downloads/")
    ...
    if args.input:
        ...
        # Try data/ first, then Downloads/
        csvs = []
        if data_dir.exists():
            csvs = list(data_dir.glob("*.csv"))
        if not csvs and downloads_dir.exists():
            csvs = list(downloads_dir.glob("*.csv"))
        
        if not csvs:
            print("No CSV files found in data/ or Downloads/.")
            sys.exit(1)
