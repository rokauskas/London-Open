...
/**
 * Download DAX 1-minute OHLC data from Dukascopy and upload to MongoDB
 * 
 * Usage:
 *   node bin/download_dax_dukascopy_1min.js --start 2024-01-01 --end 2024-12-31
 *   node bin/download_dax_dukascopy_1min.js --date 2024-11-14
 *   node bin/download_dax_dukascopy_1min.js --year 2024
 *   node bin/download_dax_dukascopy_1min.js --daily (downloads yesterday's data)
 *   node bin/download_dax_dukascopy_1min.js (same as --daily, for crontab)
 * 
 * Requirements:
 *   npm install dukascopy-node mongodb
 * 
 * Crontab setup (run daily at 1 AM):
 *   0 1 * * * /usr/bin/node /path/to/bin/download_dax_dukascopy_1min.js >> /var/log/dax-download-1min.log 2>&1
 */
...
const COLLECTION_NAME = 'ohlc_1min';
...
const LOG_FILE = path.join(LOG_DIR, 'dax-download-1min.log');
...
async function downloadDateRange(startStr, endStr) {
  ...
  // Download in daily chunks for 1-minute data to avoid overwhelming the API
  const chunks = [];
  ...
  console.log(`\nDownloading ${chunks.length} chunks (daily)...\n`);
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      // Rate limiting: wait 2 seconds between chunks (1-min data is much larger)
...
async function downloadYesterday() {
  ...
  log('='.repeat(75));
  log('DAILY AUTOMATED DOWNLOAD (1-MINUTE)');
...
async function main() {
  ...
  try {
    log('='.repeat(75));
    log('DAX DATA DOWNLOAD SCRIPT STARTED (1-MINUTE)');
  ...
  if (args.length === 0 || args.includes('--daily')) {
    ...
    try {
      if (year) {
        ...
        log('  0 1 * * * /usr/bin/node ' + __filename + ' >> /var/log/dax-download-1min.log 2>&1');
        process.exit(1);
      }
    } catch (innerError) {
      throw innerError;
